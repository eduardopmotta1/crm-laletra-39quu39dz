...
routerAdd('POST', '/api/crm/production/attach-message-file', (e) => {
  ...
  try {
    ...
    // Se falhou por conflito de unicidade (UNIQUE constraint idx_poma_order_message)
    ...
    console.error('[ATTACH MESSAGE FILE] Error creating link record:', errMsg)
    return e.json(500, {
      success: false,
      error: 'Erro ao registrar vínculo do anexo no pedido.',
    })
  }

  // 7. Vínculo criado com sucesso! Agora anexar o arquivo ao pedido de produção caso ainda não esteja
  // Copiar arquivo da mensagem para os attachments do pedido
  // Note: O frontend envia multipart se fizer upload direto, mas quando via backend endpoint,
  // podemos adicionar o arquivo no attachments do record.
  return e.json(200, {
    success: true,
    already_added: false,
    already_exists: false,
    needs_client_copy: true,
