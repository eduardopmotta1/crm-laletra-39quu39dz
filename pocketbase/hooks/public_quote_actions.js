...
routerAdd('POST', '/api/public/quotes/{token}/approve', (c) => {
  try {
    ...
    // Calcular valor total do orçamento para sincronizar no attendance
    ...
    // 3. Mover atendimento EXATO para "Venda fechada" (sem arquivar, sem criar produção, sem archived_deal)
    // "Venda fechada" indica que o cliente aprovou comercialmente. O vendedor fará a tratativa interna e decidirá quando enviar para produção.
    if (attendanceId) {
      try {
        ...
        if (att) {
          ...
          try {
            ...
            if (stageTransCollection) {
              ...
              transRec.set(
                'notes',
                'Atendimento movido para "Venda fechada" após aprovação do orçamento ' +
                  quoteCode +
                  ' pelo cliente na página pública. Valor: ' +
    ...
    try {
      ...
      if (auditCollection) {
        ...
        auditRec.set(
          ...
          'Orçamento ' +
            quoteCode +
            ' aprovado pelo cliente na página pública. Atendimento ' +
            (attendanceId || 'N/A') +
            ' movido para "Venda fechada". Valor: ' +
            formatBRL(totalValue) +
            '.',
        )
